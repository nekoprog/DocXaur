/**
 * Table builder and `section.table()` augmentation.
 * @module
 */

import { parseNumberTwips, parseImageSize, cmToEmu } from "../core/utils.ts";
import type { TableOptions, TableCellData } from "../core/docxaur.ts";
...
class TableRow {
  ...
  toXML(section: Section): string {
    let xml = "  <w:tr>\n";
    // Row height: choose max cell height (simplified)
    let maxHeight = 170;
...
class TableCell {
  ...
  toXML(colIndex: number, tableOptions: TableOptions, section: Section): string {
    ...
    if (this.data.colspan && this.data.colspan > 1) xml += `        <w:gridSpan w:val="${this.data.colspan}"/>\n`;
    ...
    if (this.data.cellColor) {
      xml += `        <w:shd w:val="clear" w:color="auto" w:fill="${this.data.cellColor}"/>\n`;
    }

    // margins (use cell → column → table fallback)
    const col = tableOptions.columns[colIndex];
    const mt = this.data.marginTop    ?? col?.marginTop    ?? tableOptions.marginTop;
    const mr = this.data.marginRight  ?? col?.marginRight  ?? tableOptions.marginRight;
    const mb = this.data.marginBottom ?? col?.marginBottom ?? tableOptions.marginBottom;
    const ml = this.data.marginLeft   ?? col?.marginLeft   ?? tableOptions.marginLeft;
    if (mt !== undefined || mr !== undefined || mb !== undefined || ml !== undefined) {
      xml += "        <w:tcMar>\n";
      if (mt !== undefined) xml += `          <w:top   w:w="${parseNumberTwips(mt)}" w:type="dxa"/>\n`;
      if (mr !== undefined) xml += `          <w:end   w:w="${parseNumberTwips(mr)}" w:type="dxa"/>\n`;
      if (mb !== undefined) xml += `          <w:bottom w:w="${parseNumberTwips(mb)}" w:type="dxa"/>\n`;
      if (ml !== undefined) xml += `          <w:start w:w="${parseNumberTwips(ml)}" w:type="dxa"/>\n`;
      xml += "        </w:tcMar>\n";
    }
    xml += "      </w:tcPr>\n";

    if (this.data.image && this.imageId !== undefined) {
      // Render inline image inside cell
      ...
      xml += `          <w:jc w:val="${jc}"/>\n`;
      xml += "        </w:pPr>\n";
      xml += `        <w:r><w:t xml:space="preserve">${(this.data.text ?? "")}</w:t></w:r>\n`;
      xml += "      </w:p>\n";
    }
...
export class Table extends Element {
  ...
  constructor(options: TableOptions) {
    ...
    if (this.options.borders === undefined) this.options.borders = true;
  }

  /** Add a row definition (built lazily). */
  ...
  /**
   * @deprecated Use direct `.row()` calls. This method will be removed.
   */
  apply(...ops: ((builder: this) => this)[]): this {
    console.warn("Table.apply() is deprecated. Use direct .row() calls instead.");
    for (const op of ops) op(this);
    return this;
  }

  private async buildRows(section: Section): Promise<void> {
    if (this.isBuilt) return;
    this.isBuilt = true;
    // Build rows from definitions, initialize images
  ...
  /** Emit OOXML for the table. */
  toXML(): string {
    throw new Error("Table.toXML() requires a Section context—call section.table(...).");
  }

  /** Internal render entry with section context. */
...
/** Module augmentation: add `table()` to Section. */
declare module "./section.ts" {
  interface Section {
    table(options: TableOptions): Table;
  }
}
(Section.prototype as any).table = function table(this: Section, options: TableOptions): Table {
  const t = new Table(options);
  // Ensure rows & images are built before rendering
  (async () => { await (t as any).buildRows(this); })();

   // Bridge element keeps Section context for rendering
